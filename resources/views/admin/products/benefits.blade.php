...
@section('adminheader')
                                    ...
                                    @if (count($benefits) > 0)
                                        @foreach ($benefits as $key => $value)
                                            <tr>
                                                ...
                                                <td>{{ $value->title }}</td>
                                                <td
                                                    style="white-space: normal !important;word-break: break-all !important; max-width: 220px;">
                                                    {{ \Illuminate\Support\Str::limit($value->desc, 150, $end = '...') }}</td>
                                                <td align="center">
                                                    <div class="row">
                                                        <div class="col-sm-6" align="right">
                                                            <a href="" class="btn btn-sm btn-warning"
                                                                ...
                                                                title="Edit Banner"
                                                                onclick="mkeEditForm('{{ $value->id }}','{{ $value->image }}','{{ $value->title }}','{{ $value->desc }}')">
                                                        ...
                                                        <div class="col-sm-6" align="left">
                                                            <form
                                                                action="{{ url('/admin/products/benefits/delete/' . $value->productid) }}"
        ...
        <div class="modal fade" id="modal-add">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form action="{{ url('/admin/products/benefits/save/' . $productid) }}" id="addform" method="POST"
                        ...
                        <div class="modal-header">
                            <h4 class="modal-title">Add Product Benefit</h4>
                        ...
                        <div class="modal-body">
                            <div class="card-body">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <textarea class="form-control" placeholder="Enter Description" name="desc" id="desc" type="text"
                                            rows="3"></textarea>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-2">
                                        <button type="button" class="btn btn-secondary btn-tooltip float-left"
                                            ...
                                            title="Image in Aspect Ratio of 4:1 Required, ie Resolution 1600 * 400px, 2000*500px ..etc.">
                                ...
                                <div class="row">
                                    <div class="col-sm-8 offset-sm-2">
                                        <img id="showImage" class="img-thumbnail hide" width="50%" height="auto"
                        ...
                        <div class="modal-footer justify-content-between">
                            ...
                            <input class="btn btn-primary offset-sm-2" type="button" onclick="validate();"
                                value="Save">
                        </div>
                    </form>
                </div>
    ...
    <script type="text/javascript">
        ...
        $('#image').change(function (e) {
          ...
          if ((file = this.files[0])) {
            ...
            img.onload = function () {
              ...
              if (width !== 715 || height !== 397) {
                $('#image').addClass('is-invalid').val('');
                $('#showImage').addClass('hide');
                $('#helpImage').html('Image dimensions must be 715px width and 397px height.').removeClass('hide');
              } else {
                $('#image').removeClass('is-invalid');
                $('#showImage').removeClass('hide');
                $('#helpImage').html('').addClass('hide');
        ...
        function mkeEditForm(id, image, title, desc) {
            $('#image').removeClass('is-invalid');
            $('#title').removeClass('is-invalid');
            $('#helpImage').html('').addClass('hide');
            $('#bannerHeading').html('Update Banner');
            $('#addform').attr('action', '{{ url('/admin/products/benefits/update/' . $productid) }}');
            $('#id').val(id);
            $('#imageOld').val(image);
            $('#title').val(title);
            $('#desc').val(desc);
