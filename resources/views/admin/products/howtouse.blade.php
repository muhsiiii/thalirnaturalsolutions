...
<div class="wrapper">
			...
			<div class="col-12">
				<div class="card card-primary">
					<div class="card-header">
						<h3 class="card-title">How To Use for Product: {{$product->name}}</h3>
					</div>
					<div class="card-body table-responsive p-0">
						<table class="table table-hover text-nowrap table-bordered table-extra">
							...
							<tbody>
								@if(count($howtouse) > 0)
									@foreach($howtouse as $key => $value)
									<tr>
										<td align="center"><?php echo $key+1; ?></td>
										<td align="center">
											<img src="{{asset($value->image)}}" width="auto" height="50px">
										</td>
										<td align="center">{{$value->step}}</td>
										...
										<td align="center">
											<div class="row">
												<div class="col-sm-6" align="right">
													<a href="" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#modal-add" title="Edit Banner" onclick="mkeEditForm('{{$value->id}}','{{$value->image}}','{{$value->step}}','{{$value->desc}}')">
														<i class="fa fa-edit" style="font-size:16px"></i> <b>Edit</b>
													</a>
												</div>
												<div class="col-sm-6" align="left">
													<form action="{{url('/admin/products/howtouse/delete/'.$value->productid)}}" method="POST" role="form">
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
      	<form action="{{url('/admin/products/howtouse/save/'.$productid)}}" id="addform" method="POST" enctype="multipart/form-data">
      		{!! csrf_field() !!}
	        ...
	        <div class="modal-header">
				<h4 class="modal-title" id="modalHeading">Add Product How to Use</h4>
	        ...
	        <div class="modal-body">
						...
						<div class="card-body">
							<div class="form-group row">
								<label for="name" class="col-sm-2 col-form-label text-right">Step: </label>
								<div class="col-sm-8">
									<input class="form-control" placeholder="Enter Step" name="step" type="number" value="" id="step">
									<span class="invalid-feedback">Please enter Step.</span>
								</div>
							</div>
...
<script type="text/javascript">
  ...
  function mkeAddForm() {
    $('#step,#id,#image,#imageOld').val('');
    $('#showImage').attr('src', '');
    $('#showImage').addClass('hide');
    $('#bannerHeading').html('Add New Banner');
    $('#image').removeClass('is-invalid');
    $('#step').removeClass('is-invalid');
    $('#helpImage').html('').addClass('hide');
  }

  function mkeEditForm(id, image, step, desc) {
    $('#image').removeClass('is-invalid');
    $('#step').removeClass('is-invalid');
    $('#helpImage').html('').addClass('hide');
    $('#modalHeading').html('Update How To Use Product');
	$('#addform').attr('action', '{{url('/admin/products/howtouse/update/'.$productid)}}');
    $('#id').val(id);
    $('#imageOld').val(image);
    $('#step').val(step);
    $('#desc').val(desc);
